...
export class AuthService {
  ...
  async validateUser(
    ...
    const userProfile = await this.usersService.findOne({ email: username }, { password: true });
  ...
  async verifyOtp(email: string, otp: string, sendMail = true) {
    ...
    if (new Date() > user.user.OTPExpired)
      throw new BadRequestException('OTP đã hết hạn');
    user.user.status = UserStatus.ACTIVE;
    //     user.OTPExpired = new Date((new Date()).getTime() - 1000); // set OTP expired
    await this.usersService.update(user.user._id as unknown as string, {
